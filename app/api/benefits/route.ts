...
export async function POST(req: NextRequest) {
  ...
  const { name, type, description, value, employee_discount, active } = body
  if (!name || !type) return NextResponse.json({ error: 'Nome e tipo obrigatórios' }, { status: 400 })

  const { data, error } = await admin.from('benefits').insert({
    company_id: me.company_id, name, type,
    description: description || null,
    value: value ?? null,
    employee_discount: employee_discount ?? null,
    active: active ?? true,
  }).select().single()
